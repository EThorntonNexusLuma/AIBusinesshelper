/* Nexus Luma AI Assistant - Clean Fullscreen Widget *//* Nexus Luma AI Assistant - Fullscreen Widget with Close Button *//* Nexus Luma AI Assistant - 380x700px Corner Widget *//* Nexus Luma AI Assistant - 380x700px Corner Widget *//* Nexus Luma AI Assistant - 380x700px Corner Widget */

(function() {

  if (window.NXL_LOADED) return;(function() {

  window.NXL_LOADED = true;

  if (window.__NXL_EMBED_LOADED__) return;(function() {

  // Create floating chat icon

  var icon = document.createElement('div');  window.__NXL_EMBED_LOADED__ = true;

  icon.innerHTML = '💬';

  icon.style.cssText = 'position:fixed;bottom:30px;right:30px;width:70px;height:70px;border-radius:50%;background:linear-gradient(135deg,#1E3FFF,#4CC3FF,#7A3FFF);color:white;font-size:32px;display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:999999;box-shadow:0 8px 32px rgba(76,195,255,0.5);transition:all 0.3s ease;border:2px solid rgba(76,195,255,0.3);user-select:none;';  if (window.__NXL_WIDGET_LOADED__) return;(function() {(function() {
  ...
  overlay.style.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(13,13,21,0.98);z-index:999998;display:none;';  var icon = document.createElement('div');  window.__NXL_WIDGET_LOADED__ = true;
  ...
  closeBtn.innerHTML = '✕';  icon.style.cssText = 'position:fixed;bottom:30px;right:30px;width:70px;height:70px;border-radius:50%;background:linear-gradient(135deg,#1E3FFF,#4CC3FF,#7A3FFF);color:white;font-size:32px;display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:999999;box-shadow:0 8px 32px rgba(76,195,255,0.5);transition:all 0.3s ease;border:2px solid rgba(76,195,255,0.3);backdrop-filter:blur(10px);user-select:none;';  if (window.__NXL_WIDGET_LOADED__) return;  if (window.__NXL_WIDGET_LOADED__) return;

  closeBtn.style.cssText = 'position:absolute;top:20px;right:20px;width:50px;height:50px;background:rgba(255,255,255,0.1);border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:24px;color:white;z-index:999999;transition:all 0.3s ease;border:2px solid rgba(76,195,255,0.3);';

  

  // Create iframe

  var iframe = document.createElement('iframe');  // Create fullscreen overlay  // Create the floating chat icon

  iframe.src = 'https://ethorntonnexusluma.github.io/AIBusinesshelper/';

  iframe.style.cssText = 'width:100%;height:100%;border:none;';  var overlay = document.createElement('div');
  ...
  function closeWidget() {    
    ...
    icon.style.display = 'flex';  // Create the iframe
  ...
  icon.onmouseleave = function() {  overlay.appendChild(closeBtn);  

    icon.style.transform = 'scale(1)';

    icon.style.boxShadow = '0 8px 32px rgba(76,195,255,0.5)';  overlay.appendChild(iframe);
  ...
  closeBtn.onmouseenter = function() {
    ...
    closeBtn.style.background = 'rgba(255,255,255,0.2)';  var isOpen = false;
  ...
  document.body.appendChild(icon);    overlay.style.display = 'block';  iframe.style.cssText = 'width:100%;height:100%;border:none;border-radius:20px;';
